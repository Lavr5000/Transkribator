...
1. **Подключиться к удаленному ПК через RDP**
   - IP: REDACTED-LAN (если в одной сети)
   - Или через Tailscale IP: REDACTED-TS-IP
   - Пользователь: User1

2. **Заменить файл на удаленном ПК**
   ```
   Удаленный ПК: C:\Users\user\.claude\REDACTED-PATH\Transkribator\TranscriberServer\AUTOSTART_SERVEO_UPDATED.bat
   ```
...
4. **Проверить работу**
   - С ноутбука выполнить: `curl http://REDACTED-TS-IP:8000/health`
...
```powershell
# С ноутбука
Copy-Item "C:\Users\user\.claude\REDACTED-PATH\Transkribator\RemotePackage\TranscriberServer\AUTOSTART_SERVEO_UPDATED.bat" `
    -Destination "\\REDACTED-TS-IP\c$\Users\user\.claude\REDACTED-PATH\Transkribator\TranscriberServer\AUTOSTART_SERVEO_UPDATED.bat" `
    -Force
```
...
$session = New-PSSession -ComputerName REDACTED-TS-IP -Credential User1
...
Copy-Item "C:\Users\user\.claude\REDACTED-PATH\Transkribator\RemotePackage\TranscriberServer\AUTOSTART_SERVEO_UPDATED.bat" `
    -Destination "C:\Users\user\.claude\REDACTED-PATH\Transkribator\TranscriberServer\" `
    -ToSession $session `
    -Force

# 4. Запустить новый сервер
Invoke-Command -Session $session -ScriptBlock {
    Start-Process -FilePath "C:\Users\user\.claude\REDACTED-PATH\Transkribator\TranscriberServer\AUTOSTART_SERVEO_UPDATED.bat" `
        -WorkingDirectory "C:\Users\user\.claude\REDACTED-PATH\Transkribator\TranscriberServer"
}
...
curl http://REDACTED-TS-IP:8000/health

# 2. Проверить порт
netstat -an | findstr 8000

# 3. Запустить транскрибатор и проверить логи
# Должно появиться сообщение: "Server healthy: http://REDACTED-TS-IP:8000"
```
