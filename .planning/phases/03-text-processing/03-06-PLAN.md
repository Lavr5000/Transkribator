...
must_haves:
  truths:
    ...
    - "Sherpa backend calls punctuation restoration (deepmultilingualpunctuation)"
    - "Whisper backend skips punctuation restoration step (already has it)"
    - "Podlodka backend calls punctuation restoration (same as sherpa)"
    - "All backends get corrections and capitalization"
    - "Punctuation restoration verified working for sherpa/podlodka backends"
  artifacts:
    - path: "src/text_processor_enhanced.py"
      provides: "Backend-aware text processing with punctuation restoration"
      contains: "backend"
      min_lines: 350
  ...
  key_links:
    ...
    - from: "src/backends/podlodka_turbo_backend.py"
      to: "src/text_processor_enhanced.py"
      via: "processor.backend='podlodka'"
      pattern: "backend.*podlodka"
---
...
Implement adaptive post-processing pipeline that adjusts behavior based on backend type. Addresses POST-06 requirement (punctuation restoration works for all backends) and POST-07 requirement (backend-aware processing).
...
<task type="auto">
  <name>Task 1: Analyze backend classes and verify deepmultilingualpunctuation integration</name>
  <files>src/backends/base.py, src/backends/whisper_backend.py, src/backends/sherpa_backend.py, src/backends/podlodka_turbo_backend.py, src/text_processor_enhanced.py</files>
  <action>
    Read backend files and text_processor_enhanced.py to understand:

    1. Check if deepmultilingualpunctuation is already integrated:
       - Search for "from deepmultilingualpunctuation import"
       - Search for "PunctuationModel" references
       - Check if _add_punctuation() method exists in EnhancedTextProcessor

    2. Base backend structure (BaseBackend class):
       - How each backend creates/uses EnhancedTextProcessor
       - Backend identification (is there a name/type property?)
       - Where text post-processing is called

    3. Current punctuation restoration status:
       - If deepmultilingualpunctuation exists: document how it's used
       - If NOT exists: note that integration is needed
    ...
    - Whether punctuation restoration is implemented
  </action>
  <verify>Backend structure documented, punctuation restoration status verified</verify>
  <done>Backend analysis complete with punctuation restoration integration status</done>
</task>
...
1. Whisper mode: Punctuation restoration skipped (verified by checking `enable_punctuation=False` when backend='whisper')
2. Sherpa mode: Full processing including punctuation restoration (verified by checking `enable_punctuation=True` when backend='sherpa')
3. Podlodka mode: Full processing including punctuation restoration (verified by checking `enable_punctuation=True` when backend='podlodka')
4. Backend detection: Each backend correctly identifies itself (test `backend_name` property)
5. Integration: All backends use EnhancedTextProcessor with backend parameter
6. Punctuation restoration test: Process "слово и еще слово" through sherpa backend -> "Слово и еще слово." (period added)

## Success Criteria

- [ ] EnhancedTextProcessor accepts backend parameter
- [ ] Whisper pipeline skips punctuation restoration (enable_punctuation=False)
- [ ] Sherpa/Podlodka pipeline includes full punctuation restoration (enable_punctuation=True)
- [ ] BaseBackend has backend_name property
- [ ] All backends pass backend to text processor
- [ ] No double punctuation in Whisper output
- [ ] Sherpa output gets punctuation added (POST-06 verified)
