...
$tailserver = "http://REDACTED-TS-IP:8000"
$serveoUrl = "http://REDACTED-TUNNEL.serveo.net:8000"

# 1. Check Tailscale
Write-Host "[1] Checking Tailscale connection..." -ForegroundColor Yellow
try {
    $ping = Test-Connection -ComputerName REDACTED-TS-IP -Count 1 -Quiet
    if ($ping) {
...
if ($serverHealthy) {
    ...
    Write-Host "1. Connect to remote PC via RDP (IP: REDACTED-TS-IP)" -ForegroundColor White
    Write-Host "2. Open file:" -ForegroundColor White
    Write-Host "   C:\Users\user\.claude\REDACTED-PATH\Transkribator\TranscriberServer\AUTOSTART_SERVEO_UPDATED.bat" -ForegroundColor DarkYellow
